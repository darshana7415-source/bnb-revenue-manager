    ...
    <div className="py-2.5 border-b border-slate-100 last:border-0">
      <div className="flex items-center gap-3">
        ...
        <div className={"text-sm font-semibold tabular-nums " + (income ? "text-emerald-700" : "text-rose-700")}>
          {income ? "+" : "−"}{fmt(t.amount).replace("Rs ", "")}
        </div>
        {pending && onMarkPaid && (
...
function AddForm({ onSave, onCancel, rooms, prefill, editTxn, incomeCats, expenseCats, onAddCategory }) {
  ...
  const submit = async () => {
    setSaving(true);
    await onSave({ id: editTxn?.id, type, amount: Number(amount), category, room: room || undefined, roomStatus: room ? roomStatus : undefined, method, date, note, status });
    setSaving(false);
  };
  ...
  return (
    <div className="px-4 pb-6">
      ...
      <label className="block text-xs text-slate-500 mb-1">Amount (LKR)</label>
...
export default function App() {
  ...
  const FIELD_LABELS = { amount: "amount", category: "category", room: "room", method: "method", date: "date", note: "note", status: "payment status" };
  ...
  const addTxn = async (t) => {
    ...
    const dbFields = { type: fields.type, amount: fields.amount, category: fields.category, room: fields.room || null, method: fields.method, txn_date: fields.date, note: fields.note || null, status: fields.status };
  ...
  const stats = useMemo(() => {
    ...
    for (const t of txns) {
      ...
      if (!paid) { t.type === "income" ? (recv += v) : (pay += v); continue; }
    ...
    return { ti, te, mi, me, recv, pay, bcomToday, bcomMonth, bcomAll };
  }, [txns]);
  ...
  const dayReport = useMemo(() => {
    ...
    return { inc: sort(inc), exp: sort(exp), ti, te, bcom, methods: Object.entries(methods), pendCount };
  }, [txns, reportDate]);
